...
namespace ArcEngine {
	...
	static void LoadArcRuntimeAssembly(const std::string& path)
	{
		if (s_AppAssembly)
		{
	...
	void ScriptEngine::Init(const std::string& assemblyPath)
	{
		...
		LoadArcRuntimeAssembly(s_AssemblyPath);
	}
	...
	static FieldType GetArcFieldType(MonoType* monoType)
	{
		...
		case MONO_TYPE_VALUETYPE:
		{
			...
			if (strcmp(name, "Arc.Vector2") == 0) return FieldType::Vec2;
			if (strcmp(name, "Arc.Vector3") == 0) return FieldType::Vec3;
			if (strcmp(name, "Arc.Vector4") == 0) return FieldType::Vec4;
		}
		}
	...
	void ScriptEngine::OnInitEntity(ScriptComponent& script, uint32_t entityID, uint32_t sceneID)
	{
		...
		// Retrieve public fields
		{
			...
			while ((iter = mono_class_get_fields(scriptClass.Class, &ptr)) != NULL)
			{
				...
				FieldType arcFieldType = GetArcFieldType(fieldType);

				// TODO: Attributes
				MonoCustomAttrInfo* attr = mono_custom_attrs_from_field(scriptClass.Class, iter);

				auto& publicField = s_PublicFields[script.ModuleName].emplace_back(name, arcFieldType);
